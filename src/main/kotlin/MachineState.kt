data class MachineState(val input: List<Int>,
                        val output: List<Int>,
                        val stateFunction: (Variable) -> Int,
                        val s: Statement)
...
fun interpret(machineState: MachineState): MachineState = with(machineState) {
    when (s) {
        is Skip -> this
        is Assign -> copy(
                stateFunction = stateFunction.andMap(s.variable, s.expression.evaluate(stateFunction)),
                s = Skip())
        is Read -> copy(
                stateFunction = stateFunction.andMap(s.variable, input.first()),
                input = input.drop(1),
                s = Skip())
        is Write -> copy(
                output = output + s.expression.evaluate(stateFunction),
                s = Skip())
        is If -> if (s.condition.evaluate(stateFunction) != 0)
            interpret(copy(s = s.trueBranch)) else
            interpret(copy(s = s.falseBranch))
        is While -> if (s.condition.evaluate(stateFunction) == 0)
            copy(s = Skip()) else
...
fun main(args: Array<String>) {
    ...
    println(interpret(factorial, listOf(20)).output)
